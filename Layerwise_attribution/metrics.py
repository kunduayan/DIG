...
def eval_log_odds(forward_fn, layer, input_embed, position_embed, type_embed, attention_mask, base_token_emb, attr, topk=20):
	logits_original						= forward_fn(input_embed, layer, attention_mask=attention_mask, position_embed=position_embed, type_embed=type_embed, return_all_logits=True).squeeze()
	...
	logits_perturbed					= forward_fn(local_input_embed, layer, attention_mask=attention_mask, position_embed=position_embed, type_embed=type_embed, return_all_logits=True).squeeze()
...
def eval_sufficiency(forward_fn, layer, input_embed, position_embed, type_embed, attention_mask, base_token_emb, attr, topk=20):
	logits_original							= forward_fn(input_embed, layer, attention_mask=attention_mask, position_embed=position_embed, type_embed=type_embed, return_all_logits=True).squeeze()
	predicted_label							= torch.argmax(logits_original).item()
	prob_original							= torch.softmax(logits_original, dim=0)
	topk_indices							= torch.topk(attr, int(attr.shape[0] * topk / 100), sorted=False).indices
	...
	logits_perturbed						= forward_fn(masked_input_embed, layer, attention_mask=masked_attention_mask, position_embed=masked_position_embed, type_embed=masked_type_embed, return_all_logits=True).squeeze()
	prob_perturbed							= torch.softmax(logits_perturbed, dim=0)

	return (prob_original[predicted_label] - prob_perturbed[predicted_label]).item()


def eval_comprehensiveness(forward_fn, layer, input_embed, position_embed, type_embed, attention_mask, base_token_emb, attr, topk=20):
	logits_original					= forward_fn(input_embed, layer, attention_mask=attention_mask, position_embed=position_embed, type_embed=type_embed, return_all_logits=True).squeeze()
	predicted_label					= torch.argmax(logits_original).item()
	prob_original					= torch.softmax(logits_original, dim=0)
	topk_indices					= torch.topk(attr, int(attr.shape[0] * topk / 100), sorted=False).indices
