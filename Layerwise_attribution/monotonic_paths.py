...
def k_nearest(int_pt, k,measure='L2_norm'):
     global word_features
     
     if measure=='cosine':
        cosine = np.dot(word_features,int_pt)/(np.linalg.norm(word_features,axis=1))
        return np.flip(np.argsort(cosine)[-k:])
     if measure=='L2_norm':
        dist = np.linalg.norm(word_features-int_pt,axis=1)
        return np.argsort(dist)[:k]
        
def linear_word_path(wrd_idx,ref_idx,steps=30,nbrs=30,strategy='greedy'):
    ...
    int_pts = [word_features[ref_idx] + (p+1)/(steps+1)*(word_features[wrd_idx]-word_features[ref_idx]) for p in range(steps)]
    
    #print(len(int_pts))
    for pt in reversed(int_pts):
        #print(pt)
        
        anchor_map = ddict(list)

        neighbors = k_nearest(pt,k=nbrs,measure='L2_norm')

        #print('neigbors:',len(neighbors))
        for idx in neighbors:
            ...
            if strategy == 'greedy':
            # calculate the distance of the monotonized vec from the anchor point
                monotonic_vec	= make_monotonic_vec(word_features[ref_idx], word_features[wrd_idx], word_features[idx], steps)
                anchor_map[idx]	= [distance(word_features[idx], monotonic_vec)]
            elif strategy == 'maxcount':
                # count the number of non-monotonic dimensions (10000 is an arbitrarily high and is a hack to be agnostic of word_features dimension)
                non_mono_count	= 10000 - monotonic(word_features[ref_idx], word_features[wrd_idx], word_features[idx], ret='count')
                anchor_map[idx]	= [non_mono_count]
            else:
                raise NotImplementedError

        if len(anchor_map) == 0:
            #print('len anchor map 0')
            word_path.append(ref_idx)

        sorted_dist_map = {k: v for k, v in sorted(anchor_map.items(), key=lambda item: item[1][0])}
        #print(sorted_dist_map)
        # remove words that are already selected in the path
        for key in word_path:
            sorted_dist_map.pop(key, None)
        #print('new sdm',sorted_dist_map)
        if len(sorted_dist_map) == 0:
            word_path.append(ref_idx)
            
        nxt_idx = next(iter(sorted_dist_map))
        wrd_idx = nxt_idx
        word_path.append(nxt_idx)
        #print(word_path)
    return word_path
...
def scale_inputs(input_ids, ref_input_ids, device, auxiliary_data, method = "UIG",steps=30, nbrs = 30, factor=0, strategy='greedy'):
    ...
    for idx in range(len(input_ids)):
        ...
        if method == "DIG":
            ...
            # print('Using UIG')
            word_path = linear_word_path(input_ids[idx], ref_input_ids[idx],steps=steps, nbrs = nbrs, strategy=strategy)
        else:
            raise NotImplementedError
